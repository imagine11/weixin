	...
	<form action="role/list" method="POST">
		<div class="text-c">
		 ...
		 <span class="select-box inline">
			...
			<input type="text" name="rname" id="" value="${roleFrom.rname }" placeholder=" 角色名称" style="width:250px" class="input-text">
			<button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 查询</button>
		</div>
	</form>	
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
	<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
	<a class="btn btn-primary radius" data-title="添加角色" data-href="role-add.html" onclick="role_add('角色添加','pages/manager/role/role_add.jsp','1','2')" href="javascript:;">
	<i class="Hui-iconfont">&#xe600;</i> 添加角色</a></span> 
	<span class="r">共有数据：<strong>${listRole.size() }</strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				...
				<tr class="text-c" >
					...
					<td>${r.id }</td>
					<td class="text-l"><u style="cursor:pointer" class="text-primary" >${r.rname }</u></td>
					<td>${r.remark }</td>
					...
					<c:if test="${r.status==0 }">
							<a style="text-decoration:none" onClick="role_start(this,${r.id})" href="javascript:;" title="上架">
								<i class="Hui-iconfont">&#xe6dc;</i>
							</a>
						</c:if>
						
						<c:if test="${r.status==1 }">
							<a style="text-decoration:none" onClick="role_stop(this,${r.id})" href="javascript:;" title="下架">
							<i class="Hui-iconfont">&#xe6de;</i>
							</a>
						</c:if>
					<a style="text-decoration:none" class="ml-5" onClick="role_edit('角色编辑','role/selectById?id=${r.id}','10001')" href="javascript:;" title="编辑">
						<i class="Hui-iconfont">&#xe6df;</i>
						</a>  
					<a style="text-decoration:none" class="ml-5" onClick="role_del(this,${r.id })" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
				</c:forEach>
			</tbody>
		</table>
	...
	layer.confirm('确认要下架吗？',function(index){
		$.ajax({
			...
			success:function(msg){
				$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="role_start(this,'+id+')" href="javascript:;" title="上架"><i class="Hui-iconfont">&#xe6dc;</i></a>');
...
function role_start(obj,id){
	layer.confirm('确认要上架吗？',function(index){
		$.ajax({
			...
			data:{"status":1,"id":id},
			success:function(msg){
				$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="role_stop(this,'+id+')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a>');
