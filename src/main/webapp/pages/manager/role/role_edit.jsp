...
<article class="page-container">
	<form class="form form-horizontal" id="form-article-add" name="form-article-add" action="role/updateTo" method="post">
	<input type="hidden" name="id" value="${role.id }">
	
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>角色名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${role.rname }" placeholder="" id="rname" name="rname">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="remark" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="$.Huitextarealength(this,200)">${role.remark }</textarea>
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">状态：</label>
				...
				<c:if test="${role.status==1 }">
					...
					<input type="hidden" value="${role.status }" name="status" id="status">
		</div>
